...
// === MAIN APP COMPONENT ===
function App() {
  // 1. CHECK IF WE ARE ON THE SCREEN URL
  ...
  // 2. NORMAL DASHBOARD LOGIC (Runs if we are NOT on /screen)
  ...
  useEffect(() => {
    ...
    if (groups.length === 0 && activeTab !== "messaging") {
      setActiveTab(null);
    }
  }, [groups, activeTab]);
  ...
  return (
    <div className="dashboard">
      ...
      <nav className="game-tabs">
        ...
        <button
          className={`game-tab ${activeTab === "messaging" ? "active" : ""}`}
          onClick={() => setActiveTab("messaging")}
        >
          <span
            className="tab-dot"
            style={{ background: "#ffff00", boxShadow: "0 0 6px yellow" }}
          />
          <span className="tab-name">MESSAGERIE</span>
        </button>
      </nav>
